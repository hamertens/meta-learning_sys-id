...
data_histories_fp = 'output_data/data_histories.csv'
# if data histories does not exist
try:
    data_histories = pd.read_csv(data_histories_fp)
    x0 = data_histories[['lr', 'epochs', 'batch_size']].values
    x0 = x0.tolist()
    y0 = data_histories['length'].values
    y0 = y0.tolist()
except FileNotFoundError:
    data_histories = pd.DataFrame({'lr': [], 'epochs': [], 'batch_size': [], "length": [], "RMSE": [], "loss": []})
    data_histories.to_csv(data_histories_fp, index=False)
    data_histories = pd.read_csv(data_histories_fp)
    x0 = []
    y0 = []
...
def train_evaluate(learning_rate, num_epochs, batch_size):
    ...
    new_row = pd.DataFrame({'lr': [learning_rate], 'epochs': [num_epochs], 'batch_size': [batch_size], 'length': [length], "RMSE": [accuracy], "loss": [loss]})
    ...
    if loss < data_histories[prev_data_histories_length:]['loss'].min():
...
# if x0 exists, use it as initial points
if len(x0) > 0:
    res = gp_minimize(
